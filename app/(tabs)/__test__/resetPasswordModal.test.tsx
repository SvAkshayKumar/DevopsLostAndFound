...
describe('ResetPasswordModal', () => {
  ...
  it('validates password requirements', async () => {
    ...
    fireEvent.changeText(
      getByPlaceholderText('RVU Email (@rvu.edu.in)'),
      'test@rvu.edu.in',
    );
    ...
    expect(
      getByText('At least one special character').props.style,
    ).toContainEqual(expect.objectContaining({ color: '#475569' }));

    // Test complete password
    fireEvent.changeText(passwordInput, 'abcdefg123!');
    expect(
      getByText('At least one special character').props.style,
    ).toContainEqual(expect.objectContaining({ color: '#10b981' }));
  });
  ...
  it('toggles password visibility', async () => {
    ...
    if (eyeIconTouchables.length >= 1) {
      ...
      console.warn(
        'Could not find eye icon touchable - component structure may have changed',
      );
