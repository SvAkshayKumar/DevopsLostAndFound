...
export default function AddItemScreen() {
  ...
  const sendNotification = async (itemTitle: string, userId: string) => {
    try {
      ...
      if (finalStatus !== 'granted') {
        console.log(
          'Push notification permissions not granted. Cannot send notifications.',
        );
        return;
      }
      ...
      try {
        ...
        if (!projectId) {
          console.error(
            'Configuration Error in sendNotification: EXPO_PUBLIC_PROJECT_ID is not defined. Cannot get/update current user token.',
          );
        } else {
          ...
          if (profileUpdateError) {
            console.error(
              'Error saving current user token to Supabase profile:',
              profileUpdateError.message,
            );
          } else {
            console.log(
              'Current user Expo token saved/updated in Supabase profile.',
            );
          }
          // --- End Profile Update ---
          ...
          if (currentUserToken) {
            ...
            if (upsertError && upsertError.code !== '23505') {
              ...
              console.error(
                'Error upserting current device token into push_tokens table:',
                upsertError.message,
              );
            } else if (!upsertError) {
              console.log(
                'Current device Expo token ensured in push_tokens table.',
              );
            } else {
              console.log(
                `Token ${currentUserToken} already exists in push_tokens table.`,
              );
            }
          }
        }
        ...
        console.error(
          'Failed during token retrieval/profile update/push_token upsert:',
          tokenError,
        );
      ...
      if (fetchError) {
        console.error(
          'Error fetching tokens from push_tokens table:',
          fetchError.message,
        );
      ...
      const recipientTokens = tokensData
        ...
        .filter(
          (token): token is string =>
            token != null && token !== currentUserToken, // Type guard for string // Ensure not null and not current user
        );
      ...
      console.log(
        `Found ${recipientTokens.length} recipient tokens from push_tokens table to notify.`,
      );
      ...
      try {
        ...
        if (!response.ok) {
          ...
          throw new Error(
            `Expo Push API request failed with status ${response.status}: ${errorBody}`,
          );
        }
        ...
        if (responseData.data && Array.isArray(responseData.data)) {
          responseData.data.forEach(async (ticket: any, index: number) => {
            ...
            if (ticket.status === 'error') {
              console.error(
                `Error sending notification to token ${targetToken}: ${ticket.message}`,
              );
              if (ticket.details?.error === 'DeviceNotRegistered') {
                console.log(
                  `Removing invalid token from push_tokens table: ${targetToken}`,
                );
                ...
                if (deleteError) {
                  ...
                  console.error(
                    `Failed to remove invalid token ${targetToken} from push_tokens: ${deleteError.message}`,
                  );
                  // Check if it's an RLS error (42501)
                  if (deleteError.code === '42501') {
                    console.error(
                      'RLS POLICY ERROR: Deleting from push_tokens failed. Check Supabase RLS policies for DELETE on push_tokens table.',
                    );
              ...
              console.log(
                `Notification ticket generated successfully for token ${targetToken}, ID: ${ticket.id}`,
              );
            }
          });
        } else {
          console.warn(
            'Unexpected response structure from Expo Push API:',
            responseData,
          );
        }
      } catch (sendError) {
      ...
      console.error(
        'Overall error in sendNotification function:',
        error instanceof Error ? error.message : error,
      );
    }
  };
    ...
    try {
      ...
      if (!user) {
        Alert.alert(
          ...
          'You must be signed in to post an item.',
        );
        setLoading(false);
        router.replace('/auth');
        return;
  ...
  const labelStyle = [
    styles.label,
    isLost ? styles.labelLost : styles.labelFound,
  ];
  const inputBorderStyle = isLost ? styles.inputLost : styles.inputFound;
  const imageButtonBorderStyle = isLost
    ? styles.imageButtonLost
    : styles.imageButtonFound;
  const imageButtonTextStyle = [
    styles.imageButtonText,
    isLost ? styles.imageButtonTextLost : styles.imageButtonTextFound,
  ];
  ...
  const cameraIconColor = isLost
    ? styles.labelLost.color
    : styles.labelFound.color;
  ...
  return (
    ...
    >
      ...
      <View style={styles.typeSelector}>
        <TouchableOpacity
          style={[
            styles.typeButton,
            isLost
              ? styles.typeButtonLostActive
              : styles.typeButtonLostInactive,
          ]}
        ...
        >
          <Text
            style={[
              styles.typeButtonTextBase,
              isLost
                ? styles.typeButtonTextLostActive
        ...
        <TouchableOpacity
          style={[
            styles.typeButton,
            !isLost
              ? styles.typeButtonFoundActive
              : styles.typeButtonFoundInactive,
          ]}
        ...
        >
          <Text
            style={[
              styles.typeButtonTextBase,
              !isLost
                ? styles.typeButtonTextFoundActive
                : styles.typeButtonTextInactive,
            ]}
          >
            Found Item
      ...
      <TextInput
        ...
        editable={!loading && !imageUploading}
        maxLength={100}
      />
...
const styles = StyleSheet.create({
  ...
  typeButtonTextBase: {
    // Base style for all type button text
    fontSize: 15,
    fontWeight: '600',
    textAlign: 'center',
  },
