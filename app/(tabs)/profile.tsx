...
export default function ProfileScreen() {
  ...
  const [selectedItemIdForFeedback, setSelectedItemIdForFeedback] = useState<string | null>(null);
  ...
  const fetchUserData = useCallback(async () => {
    try {
      ...
      if (authError || !authUser) {
        ...
        setUser(null); setItems([]); setLoading(false); setRefreshing(false);
        return;
      }
      ...
      if (profileError && profileError.code === 'PGRST116') {
        ...
        const { data: newProfile, error: createError } = await supabase
          .from('profiles').insert({ id: authUser.id, email: authUser.email }).select().single();
      ...
      const { data: itemsData, error: itemsError } = await supabase
        .from('items')
        .select(`id, title, type, status, created_at, feedback ( helper_name, rating, experience, created_at )`)
      ...
      const processedItems = (itemsData || []).map(item => {
        ...
        if (Array.isArray(item.feedback) && item.feedback.length > 0) {
          latestFeedback = item.feedback.sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime())[0];
      ...
      Alert.alert('Error Loading Data', error.message || 'Failed to load profile data.');
        ...
        },
        {
          ...
          onPress: async () => {
            try {
              ...
              Alert.alert('Sign Out Error', error.message || 'Failed to sign out from all devices.');
            }
          },
        },
      ],
  ...
  const handleUpdateProfile = async () => {
    ...
    try {
      ...
      setUser(prevUser => prevUser ? { ...prevUser, full_name: editedName.trim() || null } : null);
  ...
  const uploadImage = async (uri: string): Promise<string | null> => { // Return type
    if (!user) {
    ...
    try {
        ...
        if (updateError) {
             ...
             console.log("DB update failed, attempting to remove uploaded image:", updateError);
             Alert.alert('Error', 'Failed to update profile. Removing uploaded image...');
...
const handleDeleteAvatar = async () => {
                ...
                onPress: async () => {
                    ...
                    try {
                        ...
                        if (storageError) throw storageError;
                         console.log(`Successfully removed avatar from storage: ${filePath}`);
                        ...
                        console.log('Profile record updated successfully (avatar_url set to null).');
  ...
  const handleSubmitFeedback = async (feedbackDetails: { helperName: string; rating: number; experience: string; }) => {
    ...
    if (selectedItemIdForFeedback) {
        ...
        prevItems.map(item => {
          if (item.id === itemIdToUpdate) {
            return {
              ...item,
              status: 'resolved',
              feedback: { // Add submitted feedback details
      ...
      console.warn("handleSubmitFeedback called but selectedItemIdForFeedback was null");
    }
  };
  ...
  const handleSendEmail = async () => {
    ...
    try {
      ...
      Alert.alert('Report Sent', 'Thank you for your feedback! We will look into it.');
       setIsBugModalOpen(false); // Close modal on success
       setContactMessage(''); // Clear message field

    } catch (error: any) {
      console.log('Email sending failed:', error);
      Alert.alert('Sending Failed', error.message || 'Could not send the report. Please try again or use another contact method.');
    } finally {
      setIsSending(false);
    }
  };
  ...
  const handleEmailRedirect = () => {
    ...
    Linking.openURL(mailtoUrl).catch((err) => {
       ...
       Alert.alert('Error', 'Could not open email app. Please check if you have one configured.');
    });
  };
  ...
  const renderStars = (rating: number) => {
    ...
    for (let i = 1; i <= 5; i++) {
      stars.push(
        <Star key={i} size={14} color={i <= rating ? '#fbbf24' : '#e2e8f0'} fill={i <= rating ? '#fbbf24' : 'none'} style={styles.inlineStar} />
      );
    }
  ...
  const getFilteredItems = () => {
    ...
    switch (activeFilter) {
      case 'lost':
        filtered = items.filter(item => item.type === 'lost' && item.status === 'active');
        break;
      case 'found':
        filtered = items.filter(item => item.type === 'found' && item.status === 'active');
        break;
      case 'resolved':
      ...
      default:
        ...
        filtered = items.filter(item => item.status === 'active' || item.status === 'resolved');
        break;
    }
    return filtered;
  };
  ...
  const getEmptyStateMessage = () => {
     switch (activeFilter) {
        case 'lost': return "You have no active lost items.";
        case 'found': return "You have no active found items.";
        case 'resolved': return "You have no resolved items.";
        case 'all':
        default: return "You haven't posted any items yet, or all items are archived.";
     }
  };
  ...
  return (
    ...
    >
      ...
      <View style={styles.header}>
        <TouchableOpacity style={styles.avatarContainer} onPress={() => user.avatar_url && setSelectedImage(user.avatar_url)} disabled={!user.avatar_url}>
          {image ? <Image source={{ uri: image }} style={styles.avatar} /> : <View style={styles.avatarPlaceholder}><Camera size={24} color="#94a3b8" /></View>}
          <TouchableOpacity style={styles.avatarOverlay} onPress={() => setAvatarModalVisible(true)}><Edit2 size={14} color="#ffffff" /></TouchableOpacity>
        </TouchableOpacity>

        <View style={styles.profileInfo}>
          {isEditing ? (
            <View style={styles.editContainer}>
              <TextInput style={styles.nameInput} value={editedName} onChangeText={setEditedName} placeholder="Enter your name" placeholderTextColor="#94a3b8" autoCapitalize="words" returnKeyType="done" onSubmitEditing={handleUpdateProfile}/>
              <TouchableOpacity style={styles.saveButton} onPress={handleUpdateProfile}><Save size={18} color="#ffffff" /></TouchableOpacity>
            </View>
          ) : (
            <View style={styles.nameContainer}>
              <Text style={styles.name} numberOfLines={1} ellipsizeMode="tail">{user.full_name || 'Update Name'}</Text>
              <TouchableOpacity style={styles.editButton} onPress={() => { setEditedName(user.full_name || ''); setIsEditing(true); }}><Edit2 size={16} color="#0891b2" /></TouchableOpacity>
            </View>
          )}
        ...
        <View style={styles.headerButtons}>
          <TouchableOpacity style={styles.iconButton} onPress={() => setIsSettingsOpen(true)}><Settings size={20} color="#64748b" /></TouchableOpacity>
          <TouchableOpacity style={styles.iconButton} onPress={handleSignOut}><LogOut size={20} color="#ef4444" /></TouchableOpacity>
        </View>
      </View>
      ...
      <View style={styles.stats}>
        ...
        <View style={styles.statItem}><Text style={styles.statNumber}>{items.filter(item => item.type === 'lost' && item.status === 'active').length}</Text><Text style={styles.statLabel}>Active Lost</Text></View>
        <View style={styles.statItem}><Text style={styles.statNumber}>{items.filter(item => item.type === 'found' && item.status === 'active').length}</Text><Text style={styles.statLabel}>Active Found</Text></View>
        <View style={styles.statItem}><Text style={styles.statNumber}>{items.filter((item) => item.status === 'resolved').length}</Text><Text style={styles.statLabel}>Resolved</Text></View>
      </View>
      ...
      <View style={styles.filterContainer}>
         ...
         <TouchableOpacity
            style={[styles.filterButton, activeFilter === 'lost' && styles.filterButtonActive]}
            onPress={() => setActiveFilter('lost')}
         >
            <Text style={[styles.filterButtonText, activeFilter === 'lost' && styles.filterButtonTextActive]}>Active Lost</Text>
         </TouchableOpacity>
         <TouchableOpacity
            style={[styles.filterButton, activeFilter === 'found' && styles.filterButtonActive]}
            onPress={() => setActiveFilter('found')}
         >
            <Text style={[styles.filterButtonText, activeFilter === 'found' && styles.filterButtonTextActive]}>Active Found</Text>
         </TouchableOpacity>
         <TouchableOpacity
            style={[styles.filterButton, activeFilter === 'resolved' && styles.filterButtonActive]}
            onPress={() => setActiveFilter('resolved')}
         >
            <Text style={[styles.filterButtonText, activeFilter === 'resolved' && styles.filterButtonTextActive]}>Resolved</Text>
         </TouchableOpacity>
      </View>
               ...
               <View key={item.id} style={styles.itemCard}>
                 ...
                 <View style={styles.itemInfoColumn}>
                   ...
                   <View style={styles.itemMeta}>
                     ...
                     <Text style={[styles.itemType, item.type === 'lost' ? styles.lostBadge : styles.foundBadge]}>
                     ...
                     {item.status === 'resolved' ? (
                       ...
                       >
                         {itemFeedback ? (
                           ...
                           <>
                             ...
                             <Text style={styles.resolvedHelperName} numberOfLines={1} ellipsizeMode="tail">
                       ...
                       <Text style={[styles.itemStatus, styles.activeStatusText]}>
      ...
      <Modal visible={isSettingsOpen} transparent={true} animationType="slide" onRequestClose={() => setIsSettingsOpen(false)}>
         <TouchableOpacity style={styles.drawerOverlay} activeOpacity={1} onPressOut={() => setIsSettingsOpen(false)}>
            ...
            <TouchableOpacity activeOpacity={1} style={styles.drawer}>
                <View style={styles.drawerHeader}>
                    ...
                    <TouchableOpacity onPress={() => setIsSettingsOpen(false)} style={styles.closeButtonDrawer}><X size={24} color="#64748b" /></TouchableOpacity>
                </View>
                <TouchableOpacity style={styles.drawerItem} onPress={() => { setIsSettingsOpen(false); setIsPasswordModalOpen(true); handleSendOtp();}}><Lock size={20} color="#64748b" /><Text style={styles.drawerItemText}>Forgot Password</Text></TouchableOpacity>
                <TouchableOpacity style={styles.drawerItem} onPress={() => { setIsSettingsOpen(false); setIsResetModalOpen(true); }}><KeyRound size={20} color="#64748b" /><Text style={styles.drawerItemText}>Update Password</Text></TouchableOpacity>
                 <View style={styles.drawerSeparator} />
                 <TouchableOpacity style={styles.drawerItem} onPress={() => { setIsSettingsOpen(false); setIsBugModalOpen(true); }}><Bug size={20} color="#64748b" /><Text style={styles.drawerItemText}>Report Bug / Feedback</Text></TouchableOpacity>
                <TouchableOpacity style={styles.drawerItem} onPress={handleEmailRedirect}><Mail size={20} color="#64748b" /><Text style={styles.drawerItemText}>Contact via Email</Text></TouchableOpacity>
                <TouchableOpacity style={styles.drawerItem} onPress={handleWhatsAppContact}><MessageCircle size={20} color="#64748b" /><Text style={styles.drawerItemText}>Contact via WhatsApp</Text></TouchableOpacity>
             </TouchableOpacity>{/* End inner TouchableOpacity */}
         </TouchableOpacity>
      </Modal>

      {/* Bug Report Modal */}
       <Modal visible={isBugModalOpen} transparent={true} animationType="fade" onRequestClose={() => { if (!isSending) setIsBugModalOpen(false); }}>
         <View style={styles.modalOverlay}>
           <View style={styles.modalContent}>
             <Text style={styles.modalTitle}>Report Bug / Feedback</Text>
             <TextInput style={[styles.input, styles.textArea]} placeholder="Please describe the issue or provide your feedback..." placeholderTextColor="#94a3b8" value={contactMessage} onChangeText={setContactMessage} multiline numberOfLines={5} textAlignVertical="top" editable={!isSending}/>
             <TouchableOpacity style={[styles.button, isSending && styles.disabledButton]} onPress={handleSendEmail} disabled={isSending || !contactMessage.trim()}>
               <Text style={styles.buttonText}>{isSending ? 'Sending...' : 'Send Report'}</Text>
             </TouchableOpacity>
             <TouchableOpacity style={styles.modalCancelButton} onPress={() => { if (!isSending) setIsBugModalOpen(false); }} disabled={isSending}>
               <Text style={[styles.modalCancelButtonText, isSending && styles.disabledText]}>Cancel</Text>
             </TouchableOpacity>
           </View>
         </View>
       </Modal>
      ...
      <PasswordModals isPasswordModalOpen={isPasswordModalOpen} setIsPasswordModalOpen={setIsPasswordModalOpen} isResetModalOpen={isResetModalOpen} setIsResetModalOpen={setIsResetModalOpen} user={user} styles={styles}/>

      {/* Full Screen Image Viewer Modal */}
      <Modal visible={!!selectedImage} transparent={true} animationType="fade" onRequestClose={() => setSelectedImage(null)}>
         <TouchableOpacity style={styles.imageViewerOverlay} activeOpacity={1} onPress={() => setSelectedImage(null)}>
            {selectedImage && <Image source={{ uri: selectedImage }} style={styles.modalImage} resizeMode="contain"/>}
         </TouchableOpacity>
      </Modal>

      {/* Avatar Edit Options Modal */}
      <Modal visible={isAvatarModalVisible} transparent={true} animationType="slide" onRequestClose={() => setAvatarModalVisible(false)}>
          <TouchableOpacity style={styles.avatarModalOverlay} activeOpacity={1} onPressOut={() => setAvatarModalVisible(false)}>
             <TouchableOpacity activeOpacity={1} style={styles.avatarModalContent}>
                 ...
                 <TouchableOpacity style={styles.avatarModalOption} onPress={handleTakePhoto}><Camera size={20} color="#0891b2" style={styles.avatarModalIcon} /><Text style={styles.avatarModalText}>Take Photo</Text></TouchableOpacity>
                 <TouchableOpacity style={styles.avatarModalOption} onPress={handleImagePick}><Edit2 size={20} color="#0891b2" style={styles.avatarModalIcon}/><Text style={styles.avatarModalText}>Choose from Library</Text></TouchableOpacity>
                 {image && (<><View style={styles.avatarModalSeparator} /><TouchableOpacity style={styles.avatarModalOption} onPress={handleDeleteAvatar}><Trash size={20} color="#ef4444" style={styles.avatarModalIcon} /><Text style={[styles.avatarModalText, { color: '#ef4444' }]}>Remove Picture</Text></TouchableOpacity></>)}
                 <View style={styles.avatarModalSeparator} />
                 <TouchableOpacity style={styles.avatarModalCancel} onPress={() => setAvatarModalVisible(false)}><Text style={styles.avatarModalCancelText}>Cancel</Text></TouchableOpacity>
             </TouchableOpacity>
          </TouchableOpacity>
      </Modal>
      ...
      <FeedbackModal
          ...
          onClose={() => { setFeedbackModalVisible(false); setSelectedItemIdForFeedback(null); }}
          onSubmit={handleSubmitFeedback}
          type={items.find(item => item.id === selectedItemIdForFeedback)?.type ?? 'lost'}
...
const styles = StyleSheet.create({
  ...
  // Header
  header: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#ffffff', padding: 15, paddingTop: Platform.OS === 'ios' ? 50 : 15, borderBottomWidth: 1, borderBottomColor: '#e2e8f0' },
  ...
  avatarOverlay: { position: 'absolute', bottom: 0, right: 0, backgroundColor: 'rgba(0, 0, 0, 0.6)', borderRadius: 10, padding: 4 },
  ...
  editContainer: { flexDirection: 'row', alignItems: 'center', marginBottom: 4 },
  nameInput: { flex: 1, borderBottomWidth: 1, borderBottomColor: '#cbd5e1', paddingVertical: Platform.OS === 'ios' ? 6 : 2, marginRight: 10, fontSize: 18, fontWeight: '600', color: '#1e293b' },
  saveButton: { backgroundColor: '#0891b2', padding: 6, borderRadius: 15 },
  headerButtons: { flexDirection: 'row', alignItems: 'center' },
  iconButton: { padding: 8, marginLeft: 5 },
  // Stats
  stats: { flexDirection: 'row', justifyContent: 'space-around', backgroundColor: '#ffffff', paddingVertical: 15, borderBottomWidth: 1, borderBottomColor: '#e2e8f0', marginBottom: 10 },
  ...
  sectionHeader: { flexDirection: 'row', alignItems: 'center', paddingHorizontal: 15, paddingTop: 15, paddingBottom: 10, borderBottomWidth: 1, borderBottomColor: '#f1f5f9' },
  sectionTitle: { fontSize: 16, fontWeight: '600', color: '#334155', marginLeft: 10 },
  itemCount: { fontSize: 12, color: '#64748b', marginLeft: 'auto'}, // Optional item count style
  // Item Card
  itemCard: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingVertical: 12, paddingHorizontal: 15, borderBottomWidth: 1, borderBottomColor: '#f1f5f9' },
  itemInfoColumn: { flex: 1, marginRight: 10 },
  itemActionColumn: {},
  itemTitle: { fontSize: 15, fontWeight: '500', color: '#1e293b', marginBottom: 6 },
  itemMeta: { flexDirection: 'row', alignItems: 'center', flexWrap: 'wrap' },
  itemType: { fontSize: 10, fontWeight: 'bold', paddingHorizontal: 6, paddingVertical: 3, borderRadius: 4, marginRight: 8, overflow: 'hidden', textTransform: 'uppercase' },
  ...
  activeStatusText: { color: '#0e7490', paddingHorizontal: 6, paddingVertical: 3, borderRadius: 4, backgroundColor: '#e0f7fe' },
  resolvedStatusContainer: { flexDirection: 'row', alignItems: 'center', paddingVertical: 3, borderRadius: 4 },
  resolvedStatusText: { fontSize: 11, fontWeight: 'bold', color: '#16a34a', textTransform: 'uppercase', textDecorationLine: 'underline' }, // Added underline
  resolvedHelperName: { fontSize: 11, color: '#52525b', marginLeft: 4, textDecorationLine: 'underline' }, // Added underline
  inlineStarsContainer: { flexDirection: 'row', marginRight: 4 },
  inlineStar: { marginRight: 1 },
  resolveButton: { backgroundColor: '#0891b2', paddingHorizontal: 12, paddingVertical: 6, borderRadius: 15 },
  ...
  modalContent: { width: '90%', maxWidth: 400, backgroundColor: '#ffffff', borderRadius: 12, padding: 20, paddingTop: 40, position: 'relative' },
  modalTitle: { fontSize: 18, fontWeight: '600', textAlign: 'center', marginBottom: 20, color: '#1e293b' },
  input: { backgroundColor: '#f8fafc', borderWidth: 1, borderColor: '#cbd5e1', borderRadius: 8, paddingHorizontal: 15, paddingVertical: 12, fontSize: 16, marginBottom: 15, color: '#1e293b' },
  textArea: { minHeight: 100, textAlignVertical: 'top' }, // Use minHeight for multiline
  button: { backgroundColor: '#0891b2', paddingVertical: 14, borderRadius: 8, alignItems: 'center', marginTop: 10 },
  buttonText: { color: '#ffffff', fontSize: 16, fontWeight: 'bold' },
  disabledButton: { backgroundColor: '#a5f3fc', opacity: 0.7 },
  disabledText: { opacity: 0.7 },
  modalCancelButton: { marginTop: 10, paddingVertical: 10, alignItems: 'center' },
  modalCancelButtonText: { color: '#64748b', fontSize: 14, fontWeight: '500' },
  closeModalButton: { position: 'absolute', top: 10, right: 10, padding: 5, zIndex: 1}, // zIndex helps ensure it's tappable

  // Settings Drawer Styles
  drawerOverlay: { flex: 1, backgroundColor: 'rgba(0, 0, 0, 0.5)', justifyContent: 'flex-end' },
  drawer: { backgroundColor: '#ffffff', borderTopLeftRadius: 16, borderTopRightRadius: 16, paddingBottom: Platform.OS === 'ios' ? 30 : 20 },
  drawerHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', padding: 15, borderBottomWidth: 1, borderBottomColor: '#e2e8f0' },
  ...
  drawerSeparator: { height: 1, backgroundColor: '#f1f5f9', marginVertical: 5, marginHorizontal: 20 },

  // Image Viewer Modal
  imageViewerOverlay: { flex: 1, backgroundColor: 'rgba(0, 0, 0, 0.85)', justifyContent: 'center', alignItems: 'center' },
  modalImage: { width: '95%', height: '85%' },
  // Avatar Edit Modal
  avatarModalOverlay: { flex: 1, justifyContent: 'flex-end', backgroundColor: 'rgba(0, 0, 0, 0.5)' },
  avatarModalContent: { backgroundColor: '#ffffff', borderTopLeftRadius: 16, borderTopRightRadius: 16, paddingTop: 10, paddingBottom: Platform.OS === 'ios' ? 30 : 20 },
  avatarModalTitle: { fontSize: 16, fontWeight: '600', textAlign: 'center', paddingVertical: 15, color: '#1e293b' },
  avatarModalOption: { flexDirection: 'row', alignItems: 'center', paddingVertical: 14, paddingHorizontal: 20 },
  avatarModalIcon: { marginRight: 15 },
  avatarModalText: { fontSize: 16, color: '#334155' },
  avatarModalSeparator: { height: 1, backgroundColor: '#e2e8f0', marginVertical: 5, marginHorizontal: 20 },
  ...
  signInButton: { marginTop: 20, backgroundColor: '#0891b2', paddingVertical: 12, paddingHorizontal: 30, borderRadius: 8 },
